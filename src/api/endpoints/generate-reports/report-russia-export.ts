...
export const getRussianExportSalesOptions: RouteOptions = {
    description: "Russian Export Sales Report Generation",
    tags: ["api", "Reports"],
    notes: "Generates Russian export sales data by product and company",
    validate: {
        headers: Joi.object({
                    ...
                    Joi.object({
                        ...
                        companies: Joi.array().items(
                            Joi.object({
                                ...
                                startDate: Joi.string().optional().empty("").allow(null),
                                technology: Joi.string().optional().empty("").allow(null),
                                feedstock: Joi.string().optional().empty("").allow(null),
        ...
        try {
            ...
            for (const product of productsResult.recordset) {
                ...
                const companiesQuery = `
                    ...
                    JOIN cr_rep_russia_exports r ON c.comp_id = r.comp_id
            ...
            return h.response({
                success: true,
                data: {
                    title: "Russian Petrochemical Export Sales Report",
            ...
            logger.error("russian-export-route", `Failed to generate Russian export sales data: ${error}`);
            return h.response({
                success: false,
                message: "Failed to generate Russian export sales data",
            }).code(500);
        }
    },
};
...
async function getYearRange() {
    const maxYearResult = await executeQuery(
        "SELECT MAX(re_year) as maxYear FROM cr_rep_russia_exports"
    );
    const minYearResult = await executeQuery(
        "SELECT MIN(re_year) as minYear FROM cr_rep_russia_exports"
    );
